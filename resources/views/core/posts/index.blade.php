		    ...
		    <table class="table table-striped table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr id="form-{{ $row->pageID }}">
							<td > {{ ++$i }} </td>
							<td ><input type="checkbox" class="ids minimal-green" name="ids[]" id="del{{ $row->pageID }}" value="{{ $row->pageID }}" />  </td>
							<td>
							 	...
							 	<div class="actions">
								  ...
								  <ul class="actions-menu"  style="    list-style-type: none;margin: 0;padding-left: 0;">
									...
									<li><a href="{{ url('blog/'.$row->alias)}}" target="_blank" class="tips btn btn-sm btn-primary btn-fs" title="{{ __('core.btn_view') }}"> <i class="fa  fa-eye"></i> </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('core/posts/'.$row->pageID.'/edit?return='.$return) }}" class="tips btn btn-sm btn-success btn-fs" title="{{ __('core.btn_edit') }}"> <i class="fa  fa-edit"></i> </a></li>
									@endif
									<li class="divider" role="separator"></li>
									@if($access['is_remove'] ==1)
										 <li><a href="javascript://ajax"  onclick="SximoDeleteP('del{{ $row->pageID }}');" class="tips btn btn-sm btn-danger btn-fs" title="{{ __('core.btn_remove') }}">
