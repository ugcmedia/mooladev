<?php usort($tableGrid, "SiteHelpers::_sort"); ?>
	 ...
	 <?php echo Form::open(array('url'=>'faqs', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
        ...
        <tbody>
			...
			<tr id="form-0" >
			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->faq_id }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids minimal-green" name="ids[]" value="<?php echo $row->faq_id ;?>" id="del{{ $row->faq_id }}" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->faq_id }}" data-url="{{ url('faqs/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
					@endif		
				 	<td data-values="action" data-key="<?php echo $row->faq_id ;?>"  >
						
							<div class="actions">
								  <ul class="actions-menu" style="    list-style-type: none;margin: 0;padding-left: 0;">
								 	@if($access['is_detail'] ==1)
									<li><a href="{{ url('faqs/'.$row->faq_id.'?return='.$return)}}" class="tips action-view btn btn-sm btn-primary btn-fs" title="{{ __('core.btn_view') }}" onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false; ">
								<i class="fa  fa-eye"></i>
								</a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('faqs/'.$row->faq_id.'/edit?return='.$return) }}" class="tips action-edit btn btn-sm btn-success btn-fs" title="{{ __('core.btn_edit') }}" >
								<i class="fa  fa-edit"></i>
								</a></li>
									@endif
									<!-- <li class="divider" role="separator"></li> -->
									@if($access['is_remove'] ==1)
										 <li><a href="javascript://ajax"  onclick="SximoDeleteP('del{{ $row->faq_id }}');" class="tips action-delete btn btn-sm btn-danger btn-fs" title="{{ __('core.btn_remove') }}">
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->faq_id }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
