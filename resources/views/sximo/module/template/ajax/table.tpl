			...
			<tr id="form-0" >
			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->{key} }}">
					...
					<td ><input type="checkbox" class="ids minimal-green" name="ids[]" value="<?php echo $row->{key} ;?>" id="del{{ $row->{key} }}" />  </td>					
				 	...
				 	<td data-values="action" data-key="<?php echo $row->{key} ;?>"  >
							...
							<div class="actions">
								  <ul class="actions-menu" style="    list-style-type: none;margin: 0;padding-left: 0;">
									...
									<li><a href="{{ url('{class}/'.$row->{key}.'?return='.$return)}}" class="tips action-view" title="{{ __('core.btn_view') }}" onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false; "> <i class="fa  fa-eye"></i></a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('{class}/'.$row->{key}.'/edit?return='.$return) }}" class="tips action-edit" title="{{ __('core.btn_edit') }}" onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false; "> <i class="fa  fa-edit"></i></a></li>
									@endif
									<!-- <li class="divider" role="separator"></li> -->
									@if($access['is_remove'] ==1)
										 <li><a href="javascript://ajax"  onclick="SximoDeleteP('del{{ $row->{key} }}');" class="tips action-delete" title="{{ __('core.btn_remove') }}">
