<?php usort($tableGrid, "SiteHelpers::_sort"); ?>
	 ...
	 <?php echo Form::open(array('url'=>'managebrands', 'class'=>'form-horizontal' ,'id' =>'SximoTable'  ,'data-parsley-validate'=>'' )) ;?>
        ...
        <tbody>
			...
			<tr id="form-0" >
			  ...
			  @endif        
                ...
                <tr class="editable" id="form-{{ $row->brand_id }}">
					<td class="number"> <?php echo ++$i;?>  </td>
					<td ><input type="checkbox" class="ids minimal-green" name="ids[]" value="<?php echo $row->brand_id ;?>" id="del{{ $row->brand_id }}" />  </td>					
					@if($setting['view-method']=='expand')
					<td><a href="javascript:void(0)" class="expandable" rel="#row-{{ $row->brand_id }}" data-url="{{ url('managebrands/'.$id) }}"><i class="fa fa-plus-circle " ></i></a></td>								
					@endif		
				 	<td data-values="action" data-key="<?php echo $row->brand_id ;?>"  >
						
							<div class="actions">
								  <ul class="actions-menu" style="    list-style-type: none;margin: 0;padding-left: 0;">
								 	@if($access['is_detail'] ==1)
									<li><a href="{{ url('managebrands/'.$row->brand_id.'?return='.$return)}}" class="tips action-view btn btn-sm btn-primary btn-fs" title="{{ __('core.btn_view') }}" onclick="ajaxViewDetail('#{{ $pageModule }}',this.href); return false; ">
								<i class="fa  fa-eye"></i>
								</a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('managebrands/'.$row->brand_id.'/edit?return='.$return) }}" class="tips action-edit btn btn-sm btn-success btn-fs" title="{{ __('core.btn_edit') }}" >
								<i class="fa  fa-edit"></i>
								</a></li>
									@endif
									<!-- <li class="divider" role="separator"></li> -->
									@if($access['is_remove'] ==1)
										 <li><a href="javascript://ajax"  onclick="SximoDeleteP('del{{ $row->brand_id }}');" class="tips action-delete btn btn-sm btn-danger btn-fs" title="{{ __('core.btn_remove') }}">
                ...
                @if($setting['view-method']=='expand')
                <tr style="display:none" class="expanded" id="row-{{ $row->brand_id }}">
                	<td class="number"></td>
                	<td></td>
                	<td></td>
