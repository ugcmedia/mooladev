	...
	<div class="page-content-wrapper no-margin">
		...
		<div class="sbox">
			...
			<div class="row">
				<div class="col-md-4"> 	
					@if($access['is_add'] ==1)
					<a href="{{ url('blocks/create?return='.$return) }}" class="btn btn-default btn-sm"  
			...
			<div class="table-responsive" style="padding-bottom: 70px;">
 			{!! Form::open(array('url'=>'blocks?'.$return, 'class'=>'form-horizontal m-t' ,'id' =>'SximoTable' )) !!}
		    ...
		    <table class="table table-striped table-hover " id="{{ $pageModule }}Table">
		        ...
		        <tbody>        						
		            @foreach ($rowData as $row)
		                <tr>
							<td > {{ ++$i }} </td>
							<td ><input type="checkbox" class="ids minimal-green" name="ids[]" value="{{ $row->block_id }}" id="del{{ $row->block_id }}" />  </td>
							<td>

							 	<div class="actions">
								  <ul class="actions-menu" style="    list-style-type: none;margin: 0;padding-left: 0;">
								 	@if($access['is_detail'] ==1)
									<li><a href="{{ url('blocks/'.$row->block_id.'?return='.$return)}}" data-toggle="tooltip" class="tips action-view btn btn-sm btn-primary btn-fs" title="{{ __('core.btn_view') }}"> <i class="fa  fa-eye"></i> </a></li>
									@endif
									@if($access['is_edit'] ==1)
									<li><a  href="{{ url('blocks/'.$row->block_id.'/edit?return='.$return) }}" data-toggle="tooltip" class="tips action-edit  btn btn-sm btn-success btn-fs" title="{{ __('core.btn_edit') }}"> <i class="fa  fa-edit"></i></a></li>
									@endif
									<!-- <li class="divider" role="separator"></li> -->
									@if($access['is_remove'] ==1)
										 <li><a href="javascript://ajax"  onclick="SximoDeleteI('del{{ $row->block_id }}');" class="tips action-delete  btn btn-sm btn-danger btn-fs" data-toggle="tooltip" title="{{ __('core.btn_remove') }}">
