...
@section('title')
@if($data['brand']->meta_title != '')
  {!! $data['brand']->meta_title !!}
@else
  {!! AppClass::stringReplaceSetting(config('settingConfig.brand_meta_title'),$data['brand']->brand_name)!!}
@endif

@endsection
@section('meta')
  <meta name="description" @if($data['brand']->meta_desc != '') content= "{!! $data['brand']->meta_desc !!}" @else  content= "{!! AppClass::stringReplaceSetting(config('settingConfig.brand_meta_desc'),$data['brand']->brand_name)!!}"  @endif>
  <meta name="keywords" content="{!! $data['brand']->meta_keywords!!}">
  @php $img = AppClass::getMetaImg($data['brand'],'brand') @endphp
  <meta property="og:image" content="{{$img}}" />
  <meta property="og:title" content="{!! $data['brand']->meta_title !!}" />
  <meta property="og:url" content="{{URL::current()}}" />
  <meta property="og:description" content="{!! $data['brand']->meta_desc!!}" />
  ...
  <div class="container">
    <div class="store-main-wrapp">
      <div class="row">
        ...
        @include('public/brand/brand-partials.side-content')
        ...
        <div class="col-lg-9 col-md-8">
          <div class="store-banner">
            <div class="store-banner-cont pb-3">
                  ...
                  <div class="row">
                      <div class="col-lg-9">
                        <div class="store-title">
                          <h1 class="font-32 text-dark fw-400">{{AppClass::getHTag($data['brand']->brand_name,$data['brand']->h1_tag,'brand','h1')}} </h1>
                        </div>
                        <div class="store-meta-disc">
                          <p class="mb-2 mb-lg-0 font-12 secondary-text">
                            @if(strip_tags($data['brand'] ->main_desc) != '' || strip_tags($data['brand']->main_desc) != null)
                                   {!! mb_strimwidth($data['brand']->main_desc, 0, 182, '...') !!}
                              @endif
                        </div>
                      </div>
                        ...
                        <div class="store-brd">
                          <nav aria-label="breadcrumb p-0">
                            <ol class="breadcrumb p-0 mb-0">
                              <li class="breadcrumb-item secondary-text-dark font-13 fw-300"><a href="{{url('/')}}" class="secondary-text-dark">{{__('public/brand.home')}}</a></li>
                              <li class="breadcrumb-item secondary-text-dark font-13 fw-300"><a href="{{url('all-brands')}}" class="secondary-text-dark">{{__('public/brand.brands')}}</a></li>
                              <li class="breadcrumb-item active font-13 fw-300" aria-current="page">{{$data['brand']->brand_name}}</li>
                            </ol>
                          </nav>
                        </div>
              ...
              @php
                ...
                $totalDeals   = 0;
                $totalCoupons = 0;
                $getData = explode('|',$data['brand']->offers_count);
            ...
            <div class="st-cat-cb-list p-3 div-inner-white rounded mb-4">
              <div class="st-cat-cb-title clearfix">
                <p class="font-18 fw-700 text-dark mb-0 d-inline-block float-left">{{__('public/brand.shop_from_txt',['brand_name' =>$data['brand']->brand_name ])}}</p>
          ...
          @include('public/cashback-partials/common-popover/share-hwi-popover')
            ...
            <!-- sidebar widget content for mobile -->
              @if($mobileDetection->isMobile())
                @include('public/brand/brand-partials.side-content-mobile')
              @endif
            <!-- .sidebar widget content for mobile -->
...
<script type="text/javascript">
  ...
  var id           = "{{$data['brand']->brand_id}}";

</script>

<script type="text/javascript" src="{{asset('public_assets/js/brand.js')}}">
