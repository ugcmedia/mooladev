...
class Shuttle_Dumper_ShellCommand extends Shuttle_Dumper {
	function dump($export_file_location, $table_prefix='') {
		...
		$precommand  = 'echo "'.escapeshellarg($this->db->password).'" | mysql_config_editor set --login-path=local --host=localhost --user='.escapeshellarg($this->db->username).' --password  ';
		exec($precommand);
		exec($precommand.$command, $output, $return_val);
