...
class CommentApiTest extends TestCase
{
    ...
    public function it_can_fetch_comments_for_a_given_page()
    {
        ...
        $response = $this->json('GET', '/comments?page_id=1')
                        ->assertStatus(200);
    ...
    public function it_can_fetch_comments_for_a_commentable_entity()
    {
        ...
        $response = $this->json('GET', '/comments?commentable_id=1&commentable_type='.Page::class)
                        ->assertStatus(200);
    ...
    public function it_can_fetch_pending_comments_if_author_email_cookie_is_set()
    {
        ...
        $response = $this->call('GET', '/comments?page_id=1', [], [
            'comment_author_email' => encrypt('foo@bar.app'),
    ...
    public function it_can_paginate_comments()
    {
        ...
        $response = $this->json('GET', '/comments?page_id=1&page=3');
    ...
    public function it_can_order_comments_by_date()
    {
        ...
        $response = $this->json('GET', '/comments?page_id=1&order=latest');
        $comments = $response->json()['comments'];

        $this->assertEquals('third', $comments[0]['content']);
        $this->assertEquals('second', $comments[1]['content']);
        $this->assertEquals('first', $comments[2]['content']);

        $response = $this->json('GET', '/comments?page_id=1&order=oldest');
        $comments = $response->json()['comments'];
    ...
    public function it_can_order_comments_by_upvotes()
    {
       ...
       $response = $this->json('GET', '/comments?page_id=1&order=best');
       $comments = $response->json()['comments'];
    ...
    public function it_can_validate_the_comment_content_length()
    {
        ...
        $this->json('POST', '/comments', ['content' => 'a'])
            ->assertJson(['content' => ['The content must be at least 2 characters.']]);

        $this->json('POST', '/comments', ['content' => 'abcde'])
            ->assertJson(['content' => ['The content may not be greater than 4 characters.']]);
    }
    ...
    public function it_can_check_if_the_parent_comment_exists()
    {
        $this->actingAs($this->testUser)
            ...
            ->assertJsonStructure(['root_id', 'parent_id']);
    ...
    public function it_can_check_if_the_commentable_entity_exists()
    {
        $this->actingAs($this->testUser)
            ...
            ->assertJsonStructure(['commentable_id']);
    ...
    public function it_remembers_the_author_email()
    {
        ...
        $this->json('POST', '/comments', array_merge($author, ['content' => 'foo','page_id' => 'bar']))
    ...
    public function it_can_not_edit_a_comment_if_is_a_guest()
    {
        ...
        $this->json('PATCH', '/comments/1', ['content' => 'updated'])
            ->assertStatus(401);
    }
    ...
    public function it_can_bulk_update_comments()
    {
        ...
        $this->actingAs($this->testUser)
            ->json('POST', '/comments/bulk-update', [
                'ids' => [1, 3],
                'status' => Comment::STATUS_PENDING,
            ])
    ...
    public function it_can_delete_a_comment()
    {
        ...
        $this->actingAs($this->testUser)
            ->json('DELETE', "/comments/{$comment->id}")
            ->assertStatus(204);
    ...
    public function it_can_report_a_comment()
    {
        ...
        $this->actingAs($this->testUser)
            ->json('PUT', "/comments/{$comment->id}/report")
            ->assertStatus(204);
    ...
    public function it_can_render_comment_preview()
    {
        ...
        $this->json('POST', '/comments/preview', [
