<template>
  <div class="comment__menu float-right">
    <button @click="collapse" type="button" class="comment__collapse btn btn-link">
      <icon :type="comment.collapsed ? 'plus' : 'minus'"></icon>
    </button>

    <dropdown v-if="loggedIn && (comment.can_update || comment.can_delete || !comment.reported)" class="ml-1 d-inline">
      ...
      <template slot="items">
        ...
        <button @click="destroy" class="dropdown-item" type="button">
          {{ $t('delete_button') }}
        </button>
