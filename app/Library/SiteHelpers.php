...
class SiteHelpers
{
	...
	public static function fmenus( $position ='top',$active = '1')
	{
		$data = array();  
		$menu = self::nestedFMenu(0,$position ,$active);		
		foreach ($menu as $row) 
		{
			$child_level = array();
			$p = json_decode($row->access_data,true);
			...
			if($is_allow ==1) 
			{
				...
				$menus2 = self::nestedFMenu($row->menu_id , $position ,$active );
				if(count($menus2) > 0 )
				{	 
					$level2 = array();							 
					foreach ($menus2 as $row2) 
					{
						...
						{						
							...
							$menus3 = self::nestedFMenu($row2->menu_id , $position , $active);
							if(count($menus3) > 0 )
							{
								$child_level_3 = array();
								foreach ($menus3 as $row3) 
	...
	public static function nestedFMenu($parent=0,$position ='top',$active = '1')
	{
		$group_sql = " AND tb_menu_access.group_id ='".Session::get('gid')."' ";
		$active 	=  ($active =='all' ? "" : "AND active ='1' ");
		$Q = DB::select("
		SELECT 
			tb_front_menu.*
		FROM tb_front_menu WHERE parent_id ='". $parent ."' ".$active." AND position ='{$position}'
		 ORDER BY ordering			
		");					
		return $Q;					
	}
...
public static function alphaID($in, $to_num = false, $pad_up = false, $passKey = null)
{
	...
	public static function toForm($forms,$layout)
	{
		...
		{		
			...
			{
				if( ! isset($form['note']) ) $form['note'] = '';
				if( ! isset($form['dependency']) ) $form['dependency'] = '';
				...
				if($form['view'] != 0)
				{
					if($form['field'] !='entry_by')
					{
						...
						$hidethis = ""; if($form['type'] =='hidden' || $form['type'] =='update_timestamp' ) $hidethis ='hidethis';
						$inhide = ''; if(count($group) >1) $inhide ='inhide';
						//$ebutton = ($form['type'] =='radio' || $form['option'] =='checkbox') ? "ebutton-radio" : "";
						$show = '';
						if($form['type'] =='hidden' || $form['type'] =='update_timestamp' ) $show = 'style="display:none;"';	
				...
				<?php } ?>'; 
						...
						{	
							...
							{			
								if($form['type'] =='hidden' || $form['type'] =='update_timestamp')
								{
									$f .= self::formShow($form['type'],$form['field'],$form['required'],$form['option']);
								} else {
									$f .= $limited_start;
									if(strlen($form['note'])>1)
									$note = '<a class="adtip" data-toggle="tooltip" data-placement="bottom" title="'.$form['note'].'"><i class="fa fa-info-circle"></i></a>';
									else
									$note= '';	
								
									$f .= '					
									  <div class="form-group depen-'.$form['dependency'].' wrap-'.$form['field'].' '.$hidethis.' '.$inhide.'" '.$show .'>
	...
	public static function formShow( $type , $field , $required ,$option = array()){
		//print_r($option);print_r($required);
		...
		$show = '';
		if($type =='hidden' || $type =='update_timestamp') $show = 'style="display:none;"';	
		/*		
		...
		{ $other_condition = ' readonly '; $mandatory = "'required'=>'false'";}
		}

		switch($type)
		{
			default;
				if($required !='0' && $validation!='readonly') { $mandatory = 'required'; } else {$mandatory='';}
				if(isset($option['prefix']) && $option['prefix'] !='' or isset($option['sufix']) && $option['sufix'] !='')
				{
					...
					if($option['prefix'] !='')
						...
						{$mandatory} {$attribute} {$other_condition}   class='form-control input-sm {$extend_class}' />";
			...
			case 'color';
				$form = "<input  type='text' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' 
						{$mandatory} {$attribute} {$other_condition}   class='form-control  colorpicker' />";
				break;

			case 'text_number';
			if($mandatory) $mandatory = 'required';
				$form = "<input  type='number' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' 
						{$mandatory} {$attribute} {$other_condition}   class='form-control is_number'  />";
				break;

				case 'text_decimal';
				if($mandatory) $mandatory = 'required';
				$form = "<input  type='text' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' 
						{$mandatory} {$attribute} {$other_condition}   class='form-control is_decimal' onkeypress='return isNumberKey(event,this)'  />";
				break;
			...
			case 'maps';
				...
				<input  type='text' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' 
						{$mandatory} {$attribute} {$other_condition}   class='form-control  ' />
	...
	$form = "<input  type='hidden' name='{$field}' id='{$field}' value='{{ date(\"Y-m-d H:i:s\") }}' />";
				break;

			case 'textarea';
				if($required !='0') { $mandatory = 'required'; }
				$form = "<textarea name='{$field}' rows='5' id='{$field}' class='form-control input-sm {$extend_class}'  
				         {$mandatory} {$attribute} {$other_condition}>{{ \$row['{$field}'] }}</textarea>";
				break;
			...
			case 'select';
				...
				if($option['opt_type'] =='datalist')
				{
					...
					if(isset($option['select_multiple']) && $option['select_multiple'] ==1)
					{
						...
						$form  .= "<select name='{$field}[]' rows='5' {$mandatory} {$other_condition} multiple  class='select2 '  > ";
						$form  .= "
						<?php 
					...
					$form = "<select ".$named." rows='5' id='{$field}' class='select2 {$extend_class}' {$mandatory} {$attribute} {$other_condition} ></select>";
			...
			case 'image';
				if($required !='0') { $mandatory = 'required'; }
				 ...
				 {
					$form = "<input  type='hidden' name='{$field}' id='{$field}' value='{{ \$row['{$field}'] }}' class='{$field}    form-control input-sm ' ";
					$form .= " {$attribute} {$mandatory} />

							<span class='input-group-btn'>
                <button class='btn btn-info' data-type='mediaUpload' data-target='.{$field}' type='button'>Choose file</button>
				<button class='btn btn-danger unUploadImage' data-type='mediaUnUpload' data-target='{$field}' type='button'>Remove File</button>
            </span>
			
							<div class='{$field}_preview'></div>
							<div class='{$field}Preview'>
					 	<div class='img-preview'>
						{!! SiteHelpers::showUploadedFileCust(\$row['{$field}'],'$option[path_to_upload]') !!}
			...
			case 'radio';
				if($required !='0') { $mandatory = 'required'; }
				$opt = explode("|",$option['lookup_query']);
				$form = "<?php \$".$field." = (isset(\$row['".$field."'])) ? \$row['".$field."'] : null ; ?>";
				for($i=0; $i<count($opt);$i++) 
				{
					$checked = '';
					$row =  explode(":",$opt[$i]); 
					...
					<input type='radio' name='{$field}' value ='".ltrim(rtrim($row[0]))."' {$mandatory} {$attribute} {$other_condition}";
					$form .= "@if( \$".$field." == '".ltrim(rtrim($row[0]))."') checked=\"checked\" @endif";
			...
			case 'checkbox';
				if($required !='0') { $mandatory = 'required'; }
				$opt = explode("|",$option['lookup_query']);
				$form = "<?php \$".$field." = (isset(\$row['".$field."'])) ? explode(\",\",\$row['".$field."']) : array() ; ?>";
				for($i=0; $i<count($opt);$i++) 
				{
					...
					<input type='checkbox' name='{$field}[]' value ='".ltrim(rtrim($row[0]))."' {$mandatory} {$attribute} {$other_condition} class='{$extend_class} minimal-red' ";
					$form .= "
				...
				$form = "<?php \$".$field." = (isset(\$row['".$field."'])) ? explode(\",\",\$row['".$field."']) : array() ; ?>";
					...
					$checked = '';
					 $form .= "<div class='switch-wrap'> <label class='switch'>
				    <input type='hidden' name='{$field}' value ='N' />
					<input type='checkbox' name='{$field}' value ='Y' ";
					$form .= "
					@if(in_array('Y',\$".$field."))checked @endif 
					";
					$form .= " /> ";	
					$form .= '  <span class="slider round"></span></label></div>';
	...
	public static  function bulkForm( $field, $forms = array(), $value ='')
	{
		...
		foreach($forms as $f)
		{
			if($f['field'] == $field && $f['search'] ==1)
			{
				...
				foreach($validations as $validation)
				{
					...
					$mandatory = "data-parsley-type='url' ";
				} else if($validation =='date') {
					$mandatory = "data-parsley-required='true'";
				} else if($validation =='numeric') {
					$mandatory = "data-parsley-type='number' ";
				...
				if($validation=='readonly')
				{ $other_condition = ' readonly '; $mandatory = "";}
				}
		...
		switch($type)
		{
			...
			case 'text';			
				$form = "<input  type='text' name='".$field."{$bulk}' class='form-control input-sm' $mandatory {$other_condition}  value='{$value}'/>";
				break;

			case 'text_date';
				$form = "<input  type='text' name='$field{$bulk}' class='date form-control input-sm' $mandatory {$other_condition}  value='{$value}'/> ";
				break;
			...
			case 'text_datetime';
				...
				case 'text_number';
				$form = "<input  type='number'  name='".$field."{$bulk}' class='form-control is_number input-sm'  $mandatory {$other_condition}  id='".$field."{$bulk}' value='{$value}' />";
				break;

				case 'text_decimal';
				$form = "<input  type='text'  name='".$field."{$bulk}'  id='".$field."{$bulk}'  value='{$value}'
						{$mandatory}  {$other_condition}   class='form-control  input-sm is_decimal' onkeypress='return isNumberKey(event,this)'  />";
				break;
			...
			case 'radio';
				...
				for($i=0; $i<count($opt);$i++) 
				{
					$checked = '';
					$row =  explode(":",$opt[$i]);
					if($value==$row[0]) $checked = ' selected ';
					$opts .= "<option value ='".$row[0]."' {$checked} > ".$row[1]." </option> ";
				}
			...
			case 'textarea';
				if($required !='0') { $mandatory = 'required'; }
				$form = "<textarea name='$field{$bulk}' rows='5' id='$field{$bulk}' class='form-control input-sm '  
				         {$mandatory} {$other_condition}>{$value}</textarea>";
				break;

			case 'textarea_editor';
				if($required !='0') { $mandatory = 'required'; }
				$form = "<textarea name='$field{$bulk}' rows='5' id='editor' class='form-control input-sm editor '  
						{$mandatory} >{$value}</textarea>";
				break;	
		}
			...
			{			
				if(preg_match('/(select)/',$form['type'])) 
				{
					if($form['option']['opt_type'] == 'external') 
					{
						...
						$lookey = '';
						$whereClause = isset( $form['option']['lookup_where_clause'])? $form['option']['lookup_where_clause']:'';
						if($form['option']['is_dependency']) $lookey .= $form['option']['lookup_dependency_key'] ;
						
						$f .= self::createPreCombo( $form['field'] , $table , $key , $val ,$app, $class , $lookey,$whereClause );
	...
	public static function createPreCombo( $field , $table , $key ,  $val ,$app ,$class ,$lookey = null,$lookup_where_clause=null)
	{
		...
		if($lookup_where_clause!=null)
		{
			$where_clause = "&where=".urlencode($lookup_where_clause);
		}
		$pre_jCombo = "
		\$(\"#{$field}\").jCombo(\"{!! url('{$class}/comboselect?filter={$table}:{$key}:{$val}&values='.\$row[\"{$field}\"]) !!}{$parent_field}{$where_clause}\",
		{ ".$parent." selected_value : '{{ \$row[\"{$field}\"] }}' });
		";	
		
		/* 
			$module_nfo = self::getModuleByTable($table);
		if($module_nfo)
		{
			$clickJS = "
			var {$field}_module = '".url($module_nfo->module_name)."';
\$(\"#{$field}\").parent().append('<a id=\"{$field}_viewmore\" target=\"_blank\" href=\"'+({$field}_module)+'/{{ \$row[\"{$field}\"] }}\">CLICK HERE</a>');			

\$(\"#{$field}\").on(\"change\",function(){
				\$(\"#{$field}_viewmore\").attr(\"href\", {$field}_module+'/'+(this.value));
			});	
";
		}
		$pre_jCombo .= $clickJS; */
	...
	public static function getModuleByTable($table)
	{
		$tb_module = \DB::table('tb_module')->where('module_db', $table)->first();
		if($tb_module)
			return $tb_module;
		else
			return null;
	...
	public static function showUploadedFile($file,$path , $width = 80) {
		...
		//	echo $files ;
			$info = pathinfo($files);	
			if($info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png" || $info['extension'] == "gif" || $info['extension'] == "JPG"|| $info['extension'] == "svg") 
			{
	...
	public static function showUploadedFileCust($file,$path , $width = 80) {
		...
		//	echo $files ;
			$info = pathinfo($files);	
				...
				$path_file = str_replace("./","",$path);
				return '<p><a href="'.asset( $path_file . $file).'" target="_blank" class="previewImage">
				<img src="'.asset( $path_file . $file ).'" border="0" width="'. $width .'"  data-src="'.asset( '/uploads/images/no-image.png' ).'" border="0" width="'. $width .'" class="img-circle" /></a></p>';
			
	
		} else {
			
			return "<img  src='".asset('/uploads/images/no-image.png')."' data-src='".asset('/uploads/images/no-image.png')."' border='0' width='".$width."' class='img-circle' /></a>";
	...
	public static function formatRows( $value ,$attr , $row = null )
	{
		...
		if($format_as =='image')
		{
			...
			// FORMAT AS LINK
			$value = '<a class="btn btn-primary" href="'.url($value).'" target="_blank">'.$format_value.'</a>';
