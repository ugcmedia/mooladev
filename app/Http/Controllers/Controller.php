...
abstract class Controller extends BaseController {
	...
	function getComboselect( Request $request)
	{
		...
		if($request->ajax() == true && \Auth::check() == true)
		{
			...
			$where = (!is_null($request->input('where')) ? $request->input('where') : null);
			$limit = (!is_null($request->input('limit')) ? $request->input('limit') : null);
			$values = (!is_null($request->input('values')) ? $request->input('values') : null);
			$rows = $this->model->getComboselect($param,$limit,$parent,$values,$where);
			$items = array();
		...
		foreach($forms as $form)
		{
			...
			if( $form['type'] =='file')
			{
				if($form['required'] =='required')
				{
					...
					{
						$validation .= '|mimes:jpg,jpeg,png,gif,bmp,svg';
...
function detailviewsave( $model ,$request , $detail , $id )

	{
		...
		if($access['is_add'] == '1' && $access['is_edit'] == '1' )
		{
			...
			$total = isset($request['counter']) ? count($request['counter']): 0;
			$mkeys = array();
	...
	public function grab( $request , $args = array())
	{
		...
		if($this->info['type'] =='ajax' || $this->info['type'] =='hybrid'){
	...
	function copy( $request)
	{
        ...
        {
			if( $column->Field != $this->info['key'] && $column->Field !='default')
	...
	function RemoveFilterComma($data) {
		$filter = rtrim(str_replace(",,",",",$data),',');
		return $filter;
	}

	public function doPaginate($record,$perPage) {
			$total = count($record);

			$currentpage = \Request::get('page', 1);
			$offset = ($currentpage * $perPage) - $perPage;

			$itemstoshow = array_slice(  $record , $offset , $perPage);

			$data	= new LengthAwarePaginator($itemstoshow ,$total ,$perPage);

			$data->setPath(url('/').'/'.request()->path());
			return ['data' => $data,'totalRecord' => $total];
	}

	public function doPaginateOnly($record,$perPage,$total) {
			$currentpage = \Request::get('page', 1);
			$offset = ($currentpage * $perPage) - $perPage;

			$data	= new LengthAwarePaginator($record ,$total ,$perPage);
