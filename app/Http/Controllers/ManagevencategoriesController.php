...
use App\Models\Managevencategories;
use Illuminate\Http\Request;
use Illuminate\Pagination\LengthAwarePaginator as Paginator;
use Validator, Input, Redirect ; 

class ManagevencategoriesController extends Controller {

	protected $layout = "layouts.main";
	protected $data = array();	
	public $module = 'managevencategories';
	static $per_page	= '10';
	
	public function __construct() 
	{
		parent::__construct();
		$this->model = new Managevencategories();
		
		$this->info = $this->model->makeInfo( $this->module);
		
		Self::$per_page = $this->info['setting']['perpage'];
		...
		$this->data = array(
			...
			'pageModule'		=> 'managevencategories',
			'frontendSlug'		=> $this->info['frontend_slug'],
			'pageUrl'			=>  url('managevencategories'),
	...
	function create( Request $request , $id =0 ) 
	{
		...
		$this->data['row'] = session()->getOldInput();
		else
		...
		switch ($task)
		{
			default:
				...
				if ($validator->passes()) 
				{
					...
					if($request->input(  $this->info['key'])!=null)
						return redirect($this->module.'/'. $request->input(  $this->info['key']).'/edit' )
							->with('message',__('core.note_error'))->with('status','error')
							->withErrors($validator)->withInput();
					else
						return redirect($this->module.'/create/'.'?old=true&return=')->withInput()->with('message',__('core.note_error'))->with('status','error')
							->withErrors($validator)->withInput();
	...
	public static function display(  )
	{
		...
		$model  = new Managevencategories();
		$info = $model::makeInfo('managevencategories');
		$data = array(
		...
		if($mode == 'view')
		{
			...
			if($row)
			{
				...
				$data['id'] = $id;
				return view('managevencategories.public.view',$data);			
			}			
		} 
		else {
			...
			return view('managevencategories.public.index',$data);	
		}
