...
use App\Models\Sximo\FrontMenu;
...
class FrontMenuController extends Controller {
	...
	public function getIndex( Request $request ,$id = null  )
	{
		...
		$row = \DB::table('tb_front_menu')->where('menu_id',$id)->get();
		if(count($row)>=1)
		{
		...
		$this->data['menus']		= \SiteHelpers::fmenus( $pos ,'all');
		...
		return view('sximo.fmenu.index',$this->data);	
	}
				...
				if(isset($m['children']))
				{
					...
					foreach($m['children'] as $l)					
					{
						if(isset($l['children']))
						{
							...
							{
								$level3[] = $l2['id'];
								\DB::table('tb_front_menu')->where('menu_id','=',$l2['id'])
									->update(array('parent_id'=> $l['id'],'ordering'=>$c));
								$c++;	
							}		
						}
						\DB::table('tb_front_menu')->where('menu_id','=', $l['id'])
							->update(array('parent_id'=> $m['id'],'ordering'=>$b));	
						$b++;
					}							
				}
				\DB::table('tb_front_menu')->where('menu_id','=', $m['id'])
					->update(array('parent_id'=>'0','ordering'=>$a));
				$a++;		
			}
			return redirect('sximo/fmenu')
	...
	function postSave( Request $request, $id =0)
	{
		...
		if ($validator->passes()) {
			...
			return redirect('sximo/fmenu?pos='.$pos)
				->with('message', 'Data Has Been Save Successfull')->with('status','success');
	...
	public function getDestroy(Request $request,$id)
	{
		...
		$menus = \DB::table('tb_front_menu')->where('parent_id','=',$id)->get();
		...
		return redirect('sximo/fmenu?pos='.$request->input('pos'))
