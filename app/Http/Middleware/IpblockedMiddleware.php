...
//                echo count($ipsAllow);exit;
			...
			$uri = $request->path();
			if($uri=='maintenance' || $uri=='restric' )
				return $next($request);



              		 if( $config['cnf_maintenance']=='true' )
              		 {
              			  $ipsAllow = explode(',',preg_replace('/\s+/', '', $config['cnf_allowip']));
                          if(count($ipsAllow) >= 1 )
                          {
		...
		$uas = explode(',',preg_replace('/\s+/', '', $config['cnf_restrictrefer']));
		if( is_array($uas) )
		{
			foreach($uas as $ua )
			{

				if(stripos('pad'.$request->header('User-Agent'),$ua) )
				return redirect('restric');
			}

		}

		 $lock_count = DB::table('tb_user_login_fails')->where('ip_addr',$request->ip())->where('lock_time','>',date('Y-m-d H:i:s'))->count();
		 if($lock_count>  $config['cnf_restrictcount'])
		//	 return redirect('restric');
